...
def learning_rate_with_decay(
  ...
  boundaries = [int(batches_per_epoch * epoch + start_step) for epoch in boundary_epochs]
  # boundaries = boundaries + int(225207)             # 加上迁移学习之前的次数
  vals = [initial_learning_rate * decay for decay in decay_rates]
  # *****************************************************************
  def learning_rate_fn(global_step):
    ...
    # ********************************************************************
    # print('The value of global_step', global_step)
    # 当global_step <= boundaries[0], 取val[0], 当 golbal_step <= boundaries[1]
    # 并且大于boundaries[0]，取val[1]
    lr = tf.compat.v1.train.piecewise_constant(global_step, boundaries, vals)
    # lr = base_lr
    # *******************************************************************
    return lr
